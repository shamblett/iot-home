...
Future main(List<String> args) async {
  ...
  /// Announce and start
  print(
      'Welcome to iot-home for device ${Secrets.temperatureDeviceId} with a sample rate of $sampleRate seconds');

  /// Create our sensor and start it
  final sensor = TemperatureSensor(sampleRate);
  sensor.initialise();
  sensor.start();

  /// Create our MQTT bridge and initialise it
  final bridge = MqttBridge(Secrets.temperatureDeviceId);
  ...
  await for (SensorData data in sensor.values) {
    ...
    print(
        '${Secrets.temperatureDeviceId} value is ${data.value} at time $dateString');
